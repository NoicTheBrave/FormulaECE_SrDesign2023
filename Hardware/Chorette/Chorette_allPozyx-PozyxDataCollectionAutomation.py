...
Date: 10/30/2022
Purpose: This file is a modified version of Erik's Pozyx code used to communicate the hardware-side to the Pozyx C code via piping and has been repurposed to souly collect Pozyx data.
The intent of this program is to collect Pozyx data and to store it into a CSV file for further analysis of said system later. 

How to use: 
1) Run your controller code (manual controller code) [not required, but recommended... depending on HOW your controller works... ;)]
2) Run this program. Once this program starts, it will collect the system time and generate a CSV file where it will store Pozyx data such as X, Y, and Heading data. 
3) Control your car as you see fit for any of the tests. Furthat analysis can be preformed using the generated CSV file. 
...
if __name__ == '__main__':
	...
	#-------Make CSV File Name (Formated by time, Prevent's cross-over of data from test-to-test)-------
	now = datetime.now()
	dt_string = now.strftime("%m%d%Y-%H_%M_%S")
	filename = "Chorette_" + dt_string
	sleep(1)

	#---------Logging data-------
	with open(filename, 'w') as csvfile: #-------This section of the code -------
		csvwriter = csv.writer(csvfile) #Generate/Begin CSV file 
		...
		while True:
			...
			holder = [pos.x, pos.y, head.heading, write] #Formated data (ignore "write")

			csvwriter.writerow(holder) # Write Pozyx data to CSV file 
